...
describe('TogglAPI project methods', () => {
  ...
  it('createProject sends POST with correct URL and body', async () => {
    ...
    const result = await api.createProject(100, { name: 'New Project', billable: true });

    expect(mockFetch).toHaveBeenCalledTimes(1);
    const [url, opts] = mockFetch.mock.calls[0] as [string, any];
    expect(url).toBe('https://api.track.toggl.com/api/v9/workspaces/100/projects');
    expect(opts.method).toBe('POST');
    expect(JSON.parse(opts.body)).toEqual({ name: 'New Project', billable: true });
  ...
  it('updateProject sends PUT with correct URL and body', async () => {
    ...
    const [url, opts] = mockFetch.mock.calls[0] as [string, any];
    expect(url).toBe('https://api.track.toggl.com/api/v9/workspaces/100/projects/1');
...
describe('TogglAPI client methods', () => {
  ...
  it('createClient sends POST with correct URL and body', async () => {
    ...
    const [url, opts] = mockFetch.mock.calls[0] as [string, any];
    expect(url).toBe('https://api.track.toggl.com/api/v9/workspaces/100/clients');
    expect(opts.method).toBe('POST');
  ...
  it('updateClient sends PUT with correct URL and body', async () => {
    ...
    const [url, opts] = mockFetch.mock.calls[0] as [string, any];
    expect(url).toBe('https://api.track.toggl.com/api/v9/workspaces/100/clients/50');
    expect(opts.method).toBe('PUT');
