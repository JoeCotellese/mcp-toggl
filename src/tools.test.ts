...
function getToolBlocks(): Array<{ name: string; block: string }> {
  ...
  for (const name of toolNames) {
    ...
    let startIdx = src.indexOf(`name: '${name}'`);
    if (startIdx === -1) startIdx = src.indexOf(`name: "${name}"`);
...
describe("tool manifest", () => {
  ...
  it("exposes client archive/restore tools", () => {
    expect(toolNames).toContain("toggl_archive_client");
    expect(toolNames).toContain("toggl_restore_client");
  });

  it("toggl_update_client does not expose archived property", () => {
    const updateBlock = getToolBlocks().find(
      (t) => t.name === "toggl_update_client",
    );
    expect(updateBlock).toBeDefined();
    expect(updateBlock!.block).not.toContain("archived");
  });
...
describe("tool annotations", () => {
  ...
  it("read-only tools have readOnlyHint: true", () => {
    for (const { name, block } of toolBlocks) {
      if (readOnlyTools.includes(name)) {
        ...
        expect(hasReadOnly, `${name} should have readOnlyHint: true`).toBe(
          true,
        );
      }
    }
  ...
  it("toggl_delete_time_entry has destructiveHint: true", () => {
    const deleteBlock = toolBlocks.find(
      (t) => t.name === "toggl_delete_time_entry",
    );
  ...
  it("write tools (non-delete) have destructiveHint: false", () => {
    for (const { name, block } of toolBlocks) {
      if (writeTools.includes(name)) {
        ...
        expect(
          hasNonDestructive,
          `${name} should have destructiveHint: false`,
        ).toBe(true);
      }
    }
  });
